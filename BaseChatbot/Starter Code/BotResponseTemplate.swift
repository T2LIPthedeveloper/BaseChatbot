...
var responseDictTemplate: [String: String] = [
...
func getBotResponseTemplate(message: String) -> String {
    ...
    for key in responseDictTemplate.keys {
        if tempMessage.contains(key) {
            response = responseDictTemplate[key] ?? ""
